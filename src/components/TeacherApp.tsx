...
const NavIcon = ({ id }: { id: Tab }) => {
  ...
  if (id === "create") return <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="currentColor" strokeWidth="1.8"/><path d="M12 8v8M8 12h8" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg>;
...
const tabLabels: Record<Tab, string> = { home: "Home", requests: "Requests", create: "Create", profile: "Profile" };
...
export default function TeacherApp({ onLogout }: Props) {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      setBranch(m.branch ?? "");
      const name = m.full_name ?? user.email?.split("@")[0] ?? "F";
      setInitials(name.split(" ").slice(0, 2).map((w: string) => w[0]).join("").toUpperCase() || "F");
    });
  }, []);

  const tabs: Tab[] = ["home", "requests", "create", "profile"];
  ...
  return (
    <div className="min-h-full flex flex-col" style={{ background: "#F8FAFC", fontFamily: "'Outfit', sans-serif" }}>
      ...
      <main className="flex-1 overflow-auto">
        {tab === "home" && <TeacherHome onNavigate={setTab} />}
        {tab === "requests" && <TeacherRequests />}
        {tab === "create" && <TeacherCreate />}
        {tab === "profile" && <TeacherProfile onLogout={onLogout} />}
      </main>
